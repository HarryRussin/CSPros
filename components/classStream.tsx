...
function ClassStream({
  ...
  const addAnnouncementToStream = async () => {
    if (announcement) {
      ...
      if (docsnap) {
        ...
        console.log([
          //@ts-ignore
          { message: announcement, publisherId: session?.user?.uid },
          ...announcementcop,
        ])
  ...
  return (
    <div className="pb-10">
      ...
      {/* ADD CLASS */}
      {//@ts-ignore
      console.log(classData.students.includes(session?.user?.uid)||classData.teacherId===session?.user?.uid)}
      {
        //@ts-ignore
        classData.students.includes(session?.user?.uid)||classData.teacherId===session?.user?.uid? (
          <div className="">
            {!openannounce ? (
              <div className="relative mt-3 mb-5 rounded shadow shadow-white">
                <button
              ...
              <div className="pb-10">
                <div className="mt-3 rounded-lg bg-gray-50 p-5 text-black">
                  ...
                  <div className="mt-3 flex justify-end space-x-5 text-sm">
                    <button onClick={() => setopenannounce(false)}>
                      Cancel
                    </button>
                    <button
      ...
      {/* SHOW ANNOUNCEMENTS */}
      <div className="flex  max-h-[500px] flex-col space-y-3 overflow-auto scrollbar scrollbar-thin scrollbar-thumb-slate-300">
